...
<body class="w-full h-full bg-blue-200 md:bg-blue-300">
    ...
    <div class="grid md:place-items-start place-items-center md:ml-60 py-3">
        ...
        <a type="submit"
            class=" block text-black hover:text-white bg-white hover:bg-gray-700 font-medium rounded-lg text-base p-2 text-center "
            type="button" href="modal_addrecord.php?firstname=&lastname=&clase=Registro">
    ...
    <div class="grid place-content-end my-2 md:my-0 md:mr-28">
        <form class="flex items-center" action="" method="GET">
            ...
            <input type="text" name="search" value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>"
                class="bg-gray-50 text-gray-900 border-2 border-white text-sm block w-52 p-1"
    ...
    <table class="m-5 md:mt-2 md:ml-56 md:mr-4 md:w-9/12 text-left border-collapse lg:ml-60 shadow-lg ">
        <?php
				 ...
				 $Query = mysqli_query($connectdb,"SELECT recordID,r.idAppointment, dateRecorded, a.petName, u.user_firstname, u.user_lastname, s.serviceName, prescription, VetDoc
        ...
        if($Query->num_rows > 0){
				while($row = $Query->fetch_assoc()) {
					echo'<tr>';
                        ...
                        echo'<td class="bg-white top-0 p-1">'.$row["idAppointment"].'</td>';
